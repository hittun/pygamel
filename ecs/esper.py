...
# -*- coding: utf-8 -*-
"""
this is a ecs framework
base on github esper project, with some specific new
"""
...
class Processor(GameObject):
    ...
    default_priority = property(lambda self: getattr(self, "_default_priority", 0))
...
class World(GameObject):
    ...
    def add_processor(self, processor_instance: Processor, priority=None) -> None:
        ...
        if priority is None:
            priority = processor_instance.default_priority
    ...
    def add_prefab(self, entity: int, prefab_instance) -> None:
        """Add Perfab to the Entity

        :param entity: A specific Entity
        :param prefab_instance: A specific Perfab instance
        """
        components = prefab_instance.get_all_components()
        for component_instance in components:
            self.add_component(entity, component_instance)

        processors = prefab_instance.get_all_processors()
        for processor_instance in processors:
            self.add_processor(processor_instance, processor_instance.priority)
